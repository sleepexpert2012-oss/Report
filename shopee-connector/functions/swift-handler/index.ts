...
const SELF_URL = (Deno.env.get("SELF_URL") || "").trim();
...
Deno.serve(async (req)=>{
  ...
  if (code && shopId) {
    ...
    if (!j.access_token) return new Response("Loi lay token: " + JSON.stringify(j), {
      status: 400
    });
    ...
    return new Response(`OK - Uy quyen thanh cong cho shop ${shopId}. Gio mo smooth-responder de keo don.`, {
      headers: {
